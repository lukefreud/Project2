  ...
  output$summary <- DT::renderDataTable({
    if (input$var == "Gold Reserve") {
      ...
      final_data <- final_data |>
        select(record_date, facility_desc, form_desc, location_desc, fine_troy_ounce_qty, book_value_amt)
      return(final_data)
      ...
      final_data <- as_tibble(parsed_data$data)
      final_data <- final_data |>
        select(record_date, restmt_flag, account_desc, line_item_desc, position_bil_amt)
      return(final_data)
      ...
      final_data <- as_tibble(parsed_data$data)
      final_data <- final_data |>
        select(record_date, security_type_desc, security_class_desc, securities_redeemed_cnt, securities_redeemed_amt)
      return(final_data)
      ...
      final_data <- as_tibble(parsed_data$data)
      final_data <- final_data |>
        select(record_date, security_type_desc, security_class_desc, debt_held_public_mil_amt, intragov_hold_mil_amt, total_mil_amt)
      return(final_data)
      ...
      final_data <- as_tibble(parsed_data$data)
      final_data <- final_data |>
        select(record_date, series_cd, series_desc, bonds_issued_cnt, bonds_redeemed_cnt, bonds_out_cnt, bonds_matured_cnt, bonds_unmatured_cnt)
      return(final_data)
    }
    round <- input$round
    tab <- GermanCredit %>% 
